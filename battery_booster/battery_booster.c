...
_Bool batt_boost_ctrl_init(void)
{
	...
	/*Initialise the UM980 Power Control Pin*/
	...
	/*Initialise the LE910C1 Power Control Pin*/
    result = cyhal_gpio_init(ARDU_IO6, CYHAL_GPIO_DIR_OUTPUT, CYHAL_GPIO_DRIVE_STRONG, false);
    if (result != CY_RSLT_SUCCESS)
    {
    	return false;
    }
    ...
    if(dio_online())
    {
    	...
    	if(!cyhal_gpio_read(USER_BTN))
    	{
    		...
    		if(!cyhal_gpio_read(USER_BTN))
    		{
    			...
    			cyhal_gpio_write(ARDU_IO6, true); //This one disables the LE910C1 LDO
    			cyhal_gpio_write(ARDU_IO7, true); //This one disables the UM980 LDO
    			CyDelay(10);
    			dio_booster_enable();

    			/*Load the DIO59020 now*/
    			CyDelay(100);
    			cyhal_gpio_write(ARDU_IO6, false); //This one disables the LE910C1 LDO
    			cyhal_gpio_write(ARDU_IO7, false);//This one enables the UM980 LDO
    			return true;
    		}
    		else /*Noise?*/
    		{
